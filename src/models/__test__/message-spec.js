...
const messageFixtures1 = require('../../../fixtures/messages-fixture-1.json')
  .messages
const messageFixtures2 = require('../../../fixtures/messages-fixture-2.json')
  .messages
...
describe('The Message Model', () => {
  describe('Top Level', () => {
    it('should return the lop level Messages for a topic ', done => {
      Message.collection()
        ...
        .then(messages => {
          ...
          const fixtureCount = allMessagesFixture.filter(
            message => message.topic_id === 1
          ).length
    ...
    it('should return the lop level (null parent_id) topics when getByTopic is called', done => {
      Message.getByTopic(1).then(messages => {
        const serializedMessages = messages.map(message => message.toJSON())
        const fixtureCount = allMessagesFixture.filter(
          message => message.topic_id === 1
        ).length
